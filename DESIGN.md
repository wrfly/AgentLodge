...
**订阅走的是 Claude Code 的凭据，所以请求也按 Claude Code 的样子发。** 请求自己没带的时候，
网关补上抓包里那一套：`user-agent: claude-cli/… (external, sdk-cli)`、`x-app: cli`、
`x-claude-code-session-id`（用对话 id，没有对话就每次一个 uuid）、`x-stainless-*`(能如实
回答的用本机真值)、`anthropic-beta` 里的 `claude-code-20250219`、URL 上的 `?beta=true`,以及
system 第一块的 `x-anthropic-billing-header:` 那行。客户端自己带了就原样透传——agent 容器里
跑的本来就是 Claude Code，补的是我们自己那几个内部调用（命名对话、写摘要、拉模型列表）。

**只对 `sk-ant-oat` 补。** API key 无论谁发都按 API 账号计费，上游也不要求这些头；给一个不是
...
缺的东西，在新库上是空转。**能加就别删**：删过的那一步回滚不回去 —— 退回旧镜像时
`user_version` 已经在前面，`migrate()` 直接返回，旧代码撞上少了列的表。真要退版本得连库
一起还原（`cli/backup-db.ts` 负责那份拷贝）。第 17 步是目前唯一的删除步。**它跑在 schema.sql 之前。** schema.sql 里有建在迁移列上的索引
...
**算出来的**（`core/db/profile.ts`）——活跃时段、中断率、动了手的轮次占比、提问长度、
agent/模型/effort 分布，全是 SQL。「动了手」是 `blocks like '%"kind":"tool_use"%'`：这是
计数不是读取，不必把 JSON 解出来。

时间按 **UTC 的「周内小时」分 168 桶**，浏览器按自己的偏移旋转。服务端 `TZ` 是部署机器的
（§12），但看图的人可能在别的时区，而错八小时的作息图比没有更糟。

**读出来的**（`app/recap.ts`）——分层总结：

```
每个对话 ──→ conversations.summary（一次性，summary_upto 记覆盖到第几条）
              ↓
        所有总结 + 第一层的数字 ──→ user_portraits（画像 + 候选记忆）
```

画像读总结而不是原始消息，所以输入量与对话数成正比而不是与消息数成正比——一年的历史到这步
也只有几百行。

**标题只写一次。** 对话建立时的标题是第一条消息截出来的，那是开场白不是内容。第一轮结束
后，用开头最多 5 条提问单独要一个名字（约二三百 token 进、十几个 token 出），写进 `title`
并盖上 `title_at`。之后不再改：重命名要再花一次调用，而且会挪动用户已经在列表里认得的那个
名字。`title_custom` 挡住用户自己起的名字。

**订阅上游还要看窗口。** 5 小时窗口就是一个 session，窗口结束之后到达的第一个请求会开一个
新的。所以窗口只剩不到 1 分钟时这次命名不发，留到下一轮——那一轮是用户自己开的。按 key 计
费的上游不上报窗口，这条判断直接为否。

**没有后台扫描。** 摘要在用户打开画像页时按需补齐（`catchUp`，一次最多 25 条），画像本身
也是按钮触发的：它是解读，不是记录。定时扫全站的对话，是在为没人要看的页面花钱。

**画像不写记忆。** 它产出候选，用户点了才落进 `memory/`。这条边界是刻意的：一个覆写记忆的
批处理会污染每一次对话，而一份读错了的报告只是读错了。
...
| 通道 | 上游给的 | 我们给的 |
|---|---|---|
| `/v1/messages` 响应头 `anthropic-ratelimit-unified-*` | 池子的 5h / 7d 利用率 | 丢弃。改写成该用户配额换算的一组同名头；他没配额的窗口，换成他在池子里的份额 |
...
```
边界   上游给的真实 reset —— 5 小时取 `5h`、周取 `7d`，谁看都一样
数量   这个用户自己在这段区间里的消耗（usage_records 按 created_at 过滤）
分母   这个用户在这个窗口上的配额（user_quotas.window_limit / week_limit）
```
...
上游还没回过任何响应时（网关刚重启、假上游）回落到后台配的周期：5 小时按锚点小时切一条
全局网格，周就是那个日历周。周这条是后补对齐的——它以前**只有**日历周，所以一个部署第一次
收到上游 `7d` 读数时周边界会挪一次，周用量跟着跳变一次。这是有意换的：下面那个折算要求
分子分母落在同一个窗口里，而日历周对不上上游的滚动周。

**没有配额的用户，报的是他在池子里的份额。** 「不设限」是本地的说法：没人给他配上限，他仍
在同一份订阅里，池子空了照样被拒。而一个没有上限的窗口发不出头，没有头 CLI 也不发
`rate_limit_event`——这类用户是**一头撞上去**的，事先零提示。

池子自己的利用率不能直接给：那是全平台的消耗，转发出去等于把整个平台的生意讲给一个租户听。
给的是他在里面的份额：
...
（份额不超过 1）；读的人只知道分子，分母和池子利用率都反推不出来——所以不泄漏池子状态，
**不需要管理员门禁**，每个这样的用户都发。

`utilization` 给他自己的，`status` 给池子的：两个字段各答各的问题。少了后者，CLI 在池子撞墙前
拿不到任何能触发 `rate_limit_event` 的东西；代价是漏出一个三档的粗信号（平台快满了）。

`GET /api/oauth/usage` 不参与折算：它没有请求级的 provider，不知道该按哪条上游算，不设限的
窗口继续回 `null`。代码在 `gateway/pool-share.ts`。
...
**没有手动清零**（曾经有，已移除）。它把某个人的计数起点往窗口内部挪，于是那个人和其他
所有人不在同一把尺子上，而闸门对别人照旧；网页上还得同时显示「闸门算的」和「实际花的」
两个数才说得清。临时放行某个人用充值——它抬的是上限而不是藏消费，而且在窗口自己的边界
过期。
...
- 单个用户：改三个上限、给某个窗口充值、暂停账号、强制回收容器、查看其会话与消费明细
- 批量：给一组用户设置统一配额
- 邀请码：生成 / 撤销 / 查看使用情况
