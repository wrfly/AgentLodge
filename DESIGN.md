...
**标题跟着总结一起写。** 对话刚建立时标题只能从第一条消息截，那是开场白不是内容。总结那
一次调用同时返回标题，所以改名不额外花钱。`title_custom` 挡住用户手动改过的：自动标题比
开场白准，但仍然是猜的，用户起的名字不是。

**总结由扫描驱动，不由"一轮结束"驱动。** 每 30 分钟一次，挑安静了 15 分钟以上、且
`summary_upto` 落后于消息数的对话。十分钟后回来接着聊的还是同一个对话，按轮触发等于为
同一个对话反复付钱。一次总结大约是被总结那个对话的 1% 开销，所以它不需要开关。

画像那一步是按钮触发的：它是解读，不是记录，多一个对话也不会让结论变。
