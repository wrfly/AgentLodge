...
用量按 token 记的时候，默认所有模型一视同仁：一个 token 就是一个 token，乘上后台那两个固定
权重（缓存读 0.1、输出 1.5）。**同一家厂商的价格能差十倍**——Fable 5.1 是每百万 \$10/\$50，
Haiku 4.5 是 \$1/\$5——所以跑贵模型的人花掉十倍的钱，扣掉一样多的配额。

后台「系统设置 → 配额」里的**「按哪个模型的价格折算 token」**解决这件事。留空是原来的行为；
填一个模型名（比如 `claude-opus-5`）之后，每一轮先按价格表算出金额，再除以那个模型一个输入
token 的价格。于是「一个计费 token」= 「基准模型的一个输入 token」，贵的模型自动多扣。

留空是默认值，因为打开它会改变现有额度的含义：输出 token 从 1.5 倍变成按真实价格（通常是输入的
5 倍），已经设好的上限会一下子变紧。要开就连额度一起重设。

价格表是唯一的数据源，这也是它能表达固定权重表达不了的东西的原因。举一个真实的：

| 模型 | 输入 | 输出 | 缓存读 |
|---|---|---|---|
| Claude Fable 5.1 | \$10 / MTok | \$50 | **\$0.25**（输入的 1/40） |
| Claude Opus 5 | \$5 | \$25 | \$0.5（输入的 1/10） |
| Claude Sonnet 5 | \$2 | \$10 | \$0.2 |
| Claude Haiku 4.5 | \$1 | \$5 | \$0.1 |

别的模型缓存读都是输入价的十分之一，Fable 是四十分之一——一个全局的「缓存读权重」写不出这个差别，
一张价格表可以。

**没有价格行的模型退回用权重，不会变成免费。** 基准模型自己没有价格行也一样退回。价格表填错
不该让某个模型白跑。

⚠️ 价格表默认只种了 DeepSeek 和一个 `*` 兜底，币种取 `billing.currency`（默认 CNY）。
用 Anthropic 的模型要自己按上表加行，**并且整张表的币种要统一**——金额是直接相加的。
