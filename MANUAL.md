...
这样价格表和配额才测得出区别。实测两轮对话记到 ¥0.0431。
...
扛不住有人把整个卷拿走；要那份保证就在 `.env` 里给 `CREDENTIAL_MANAGER_KEY`。发布镜像里
它是默认服务，没有它任何上游都没法鉴权。
...
access token，把它放进库里、让它经过浏览器，等于把订阅本身放在那儿。所以它只由
credential-manager 持有，网关每次发请求前问它要一个几小时寿命的 access token，过期前它自己
就把新的换好。
...
（5 小时取 `5h`、周取 `7d`，所有人一致），分子是这个用户在这个窗口里的消耗，分母是他
在这个窗口上的配额（`window_limit` / `week_limit`）。

这一点很重要：上游 2 点重置、窗口到 7 点，某人 4 点才发第一条消息——按他自己算窗口到 9 点，
可 7 点池子就空了，他会带着没用完的配额被拒。边界必须是平台的。

上游还没回过响应时（网关刚重启、用的是假上游）回落到后台配的周期本身：5 小时窗口按锚点
小时切一条全局网格，周就是后台配的那个日历周。**这个回落只用来算数字，不用来拦人**：
从来没有上游报过这两个窗口的部署（比如只配了 Cursor 的），闸门只按月度判定，
见 `§ 配额与用量`。

> **升级提示**：周窗口的边界从「后台配的日历周」改成上游 `7d` 的滚动周。第一次收到上游
> `7d` 读数时，所有用户的周边界会挪一次，落在窗口里的记录跟着变，周用量会跳变一次。
> 从没收到过读数的部署维持原样，一个字节不差。

### 没设上限的用户，看到的是自己在池子里的份额

「不设限」是本地的说法。没人给他配上限，可他仍在同一份共享订阅里，池子空了照样被拒——
以前是一头撞上去：没有上限就不发那组响应头，没有响应头 CLI 也不发 `rate_limit_event`，
事先零提示。

这类窗口改用全局用量折算：

```
个人利用率 = (这个用户的用量 ÷ 全平台的用量) × 上游报的池子利用率
```

两个用量取同一个窗口、同一条上游（多上游部署按 `provider_id` 过滤，否则分母被撑大、
份额算小）。左边那项是无量纲的份额，量纲相消，所以我们价格表里的价钱不必跟 Anthropic
自己的计价口径一致——汇率出现在分子上，也同样出现在分母上。

- 平台上只有他一个人 → 恰好等于池子的利用率，因为份额是 1
- 谁都不会超过池子的利用率，因为份额不超过 1
- 他只看得到分子，推不出分母，也推不出池子的利用率——不泄漏池子状态，不需要管理员门禁

`utilization` 是他自己的，`status` 直接取池子的：数字是你的，预警是池子的。代价是漏出一个
三档的粗信号（平台快满了），换来撞墙前有提示。

上游还没回过响应、或者这条上游没报过这个窗口时，什么都不发，跟今天一样；有上限的窗口
一个字节不改，走上面那条。`GET /api/oauth/usage` 不参与折算——它没有请求级的 provider，
不知道该按哪条上游算，不设限的窗口继续回 `null`。
...
等值查找，慢哈希会逼着全表扫描。撤销立即生效，key 仍留在列表里，能看到它用了多少。撤销过的 key 可以删除，删掉后它的用量照样算在总量和配额里。
...
一个自包含的 HTML：内联样式和脚本、中英双语（跟随浏览器，右上角一键切换）、深浅色跟随
系统，没有构建步骤。资源引用一律相对路径——Pages 把它放在仓库名的子路径下，绝对路径会 404。

- **登录按钮靠探测决定显不显示**：页面 `HEAD ./app.json`，这个路径由本部署的 caddy 直接
  应答（`{"app":true}`），Pages 上不存在。所以同一个文件在这边有登录入口、在那边没有。
  用独立路径而不是借 `/api/hello`：那个是 Claude Code 的存活探测端点，借来用会让每一次
  介绍页访问都长得像一个 CLI 在报到。相对路径，问的是"谁给了我这个页面"，而不是主机根目录
...
- **滚动的那两个只在上游真有的时候才强制**：5 小时和周来自 Anthropic 的
  `anthropic-ratelimit-unified-*` 响应头，Cursor 订阅没有这个概念（它是月度美元池）。
  没有任何上游报过这两个窗口时，闸门只按月度拦人，后台那两个输入框置灰标「未生效」——
  填过的数留在库里、页面照报，等配了有这种节奏的订阅（**第一个响应经过网关之后**）
  自动重新生效。证据 30 天不刷新就作废，免得换掉 Claude 订阅的部署永远留着一个
  没人补充的相位锚点。判断依据只能是「有没有报过」，不能看上游 `kind`：
  Claude 订阅和 DeepSeek 都是 `anthropic-native`
...
- **后台里额度一律以结算币种的整数单位输入**：用户额度、邀请码预置额度、充值、
  新用户默认额度，四处一致。`10000000` 里少一个零看不出来，`10` 里少一个看得出来。
  存储、计费、比较都是百万分之一单位 —— 只有输入框换算，服务端一个字节都没变
...
- **没有「清零某个用户的用量」这个操作**（曾经有，已移除）：窗口边界全平台统一，
  把一个人的计数起点往后挪就等于让他单独用另一把尺子。要临时放行某个人用充值
- **上限就是金额**，折成结算币种，这是全系统唯一用到汇率的地方。曾经还有一种按 token 的
  口径，已移除：一个 token 计数承载不了价格（`§ 价格与计费`）
...
- **标题只生成一次**：一开始是第一条消息截出来的（「hello」「pin」），第一轮结束后用开头
  最多 5 条提问单独要一个名字，之后就不再改了。**你自己重命名过的不动**（`title_custom`）
- **用订阅时避开窗口末尾**：5 小时窗口就是一个 session，窗口只剩不到 1 分钟时这次命名不发，
  等你下一轮消息带起新窗口再说
- **摘要是按需的**：打开画像页时才补齐没总结的对话（一次最多 25 条），存在
  `conversations.summary`,对话又长了（`summary_upto` 小于当前消息数）才重做。没有后台扫描
...
- **并发闸门**：上限热调且落库（重启不丢），「跟随上游自适应」开关决定 AIMD 砍不砍并发，
  按上游分池显示各自在途与排队
- 系统设置：邮件服务与发件人 / 站点地址（带「发测试邮件」按钮）、新用户默认额度、结算币种、1 美元 = 多少人民币
...
**金额**按价格表逐个模型算，每家厂商用自己发布的币种：Anthropic 报美元，DeepSeek 报人民币，
**表里和每条用量记录里都按原币存**（记录带一个 `cost_currency`）。价格表不做换算——换算过的
数字对不上厂商账单，而且汇率一动就过期了。

**送到界面上时统一折成结算币种**（后台 → 配额 → 结算币种）。报表曾经原样印出「¥12.34 + \$5.67」，
诚实但没法用：一眼看不出这比上个月多还是少，而且每一列金额实际占两列宽。现在主数字只有一个，
鼠标悬停能看到它是由哪几种币凑出来的。

**汇率只有一个数**：`1 美元 = 多少人民币`（后台 → 配额 → 1 美元 = 多少人民币）。两家厂商、两种币，
一个数就够了，结算币种设成哪边它都能用。它在**读取时**生效，对老数据同样生效——所以填错了改一下
就全对了，不会留下一个月的错数字等着写脚本重算。代价是「上月账单」不是一个冻结的数。

**配额就是金额**，三个窗口各一个上限，单位同样是结算币种，跟报表用的是同一个数。

曾经还有一种「按 token」的上限，算的是输入 + 缓存读×0.1 + 缓存写 + 输出×1.5。两家厂商开始
用两种币之后这条路断了：一个 token 计数承载不了价格。按各自币种的兜底行去除，DeepSeek 最便宜的
模型和 Opus 扣得一样多（它*就是*人民币兜底行）；换算着除，则每个人的配额会随汇率漂。所以现在
只有金额一种，账单也就是 `输入×输入价 + 缓存读×缓存读价 + 缓存写×缓存写价 + 输出×输出价`，
没有中间那一层。

种子价格表，每家按自己的币种，每百万 token：

| 模型 | 输入 | 输出 | 缓存读 | 缓存写 |
|---|---|---|---|---|
| Claude Fable 5.1 / 5 | \$10 | \$50 | **\$0.25**（输入的 1/40） | \$12.5 |
| Claude Opus 5 / 4.8 | \$5 | \$25 | \$0.5（输入的 1/10） | \$6.25 |
| Claude Sonnet 5 | \$2 | \$10 | \$0.2 | \$2.5 |
| Claude Haiku 4.5 | \$1 | \$5 | \$0.1 | \$1.25 |
| `*` 兜底（USD） | \$5 | \$25 | \$0.5 | \$6.25 |
| DeepSeek V4.1-Flash | ¥1 | ¥4 | ¥0.02 | ¥1 |
| DeepSeek V4-Pro | ¥4.5 | ¥13.5 | ¥0.15 | ¥4.5 |

缓存读那一列是重点：别的模型都是输入价的十分之一，**只有 Fable 5.1 是四十分之一**（Fable 5
不是，官方文档的脚注写明只有 5.1 和 Mythos 5.1 走 0.025×）。一个全局的「缓存读权重」写不出
这个差别，一张价格表可以。Anthropic 的缓存写是通行的 1.25 倍输入价；DeepSeek 不额外收费，
未命中就是输入价。DeepSeek 两个型号的数字是**空闲时段**价，工作日 UTC 01–04、06–10 翻倍。
...
- **价格表算不出来就是 0**，所以上游不是这两家的一定要自己加行——否则那部分流量既记不出账，也不扣任何人的配额。
- 种子里**缺哪行补哪行**，不覆盖你改过的行。补进去的行 `effective_from` 回填到 1970，这样历史记录也能按它重新计价；否则「现在才生效」的价格对过去的账单不可见，重算会一条都改不动（实测：66 条，0 条变化）。
- **历史金额会重算一次**（启动时，只跑一次）。token 数一个不动，只有金额是派生的，重新派生就是全部动作。日志里会打印改了多少条、前后各是多少。
...
邀请邮件、密码重置和额度提醒共用一套配置，在**系统设置**里选：

| | 要填什么 | 免费额度（2026 年 8 月） |
|---|---|---|
| `resend` | API Key | 3,000 封/月，100 封/天 |
| `brevo` | API Key | 300 封/天 |
| `smtp` | 地址、端口、用户名、密码 | 看你用的是哪台中继 |

发件地址必须在服务商那边验证过（整个域名或单个地址都行），否则邮件会被拒收或判成垃圾邮件。
SMTP 的端口决定加密方式：587 和 25 先明文连上再用 STARTTLS 升级，465 从第一个字节就是 TLS。
填完点「发测试邮件」，它往当前管理员的邮箱发一封。

一个都不配也能跑：邀请链接和重置链接会打到服务端日志里，本地开发不需要在任何服务商开户。
...
| `MAIL_PROVIDER` | `resend` | `resend`、`brevo` 或 `smtp`。这一行和下面几行都只是后台设置的环境兜底 |
...
把第一个管理员的邀请码抠出来打给你。它会问地址和端口（默认 `localhost:8080`），也可以在管道里直接给：
`curl -fsSL https://u.kfd.me/X7 | SITE=lodge.example.com PORT=9000 sh`。`TAG=` 换镜像通道，
`START=0` 只写文件不启动，方便先看一眼 `.env`。
...
| | |
|---|---|
| **不设置** | 每次重启随机生成 → 后台填的邮件 key **永久解不开**，得重填 |
...
5. 把**价格表**核对一遍（默认值是占位数，按官网单价改）。同一个模型挂了两条上游、价格不同时，
   价格行要指定上游
6. 需要邮件邀请就在**系统设置**里选一个邮件服务（Resend / Brevo / SMTP）填上，页面上有「发测试邮件」按钮
