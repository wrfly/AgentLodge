...
表里就这么存。不做换算——换算过的数字对不上厂商账单，而且汇率一动就过期了。每条用量记录带
一个 `cost_currency`，所以**两种币从不相加**，界面上显示成「¥12.34 + \$5.67」。

**配额就是金额**，三个窗口各一个上限，单位是结算币种。上限是一个数，所以两种币要折成
**结算币种**（后台 → 配额 → 结算币种 / 汇率）——这是全系统**唯一**出现汇率的地方，而且只影响
从设置那一刻起的计数，不会重算历史账。
...
- **每行自带币种**，各家用各家的。金额按币种分开汇总，从不相加；只有按金额限额时才折成结算币种。
