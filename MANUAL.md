...
份额算小）。左边那项是无量纲的份额，量纲相消，所以我们的 token 权重
（`quota.weightOutput` 那几个）不必跟 Anthropic 的计价口径一致。
...
- **上限有两种口径**，按用户选：按 token（输入 + 缓存读×0.1 + 缓存写 + 输出×1.5；缓存读和输出
  两个权重可在后台改，不涉及币种）或按金额（折成结算币种，是全系统唯一用到汇率的地方）
...
- 系统设置：邮件服务与发件人 / 站点地址（带「发测试邮件」按钮）、新用户默认额度、计费权重
...
- **按 token**：输入 + 缓存读×0.1 + 缓存写 + 输出×1.5（缓存读和输出两个权重可在后台改）。
  纯 token 口径，不涉及任何币种，也不随价格变动。
- **按金额**：上限是一个数，所以要把两种币折成**结算币种**（后台 → 配额 → 结算币种 / 汇率）。
  这是全系统**唯一**出现汇率的地方，而且只影响从设置那一刻起的计数，不会重算历史账。

计费 token 曾经是「金额 ÷ 兜底行输入价」，让贵模型自动多扣配额。两家厂商开始用两种币之后
这条路断了：按各自币种的兜底行去除，DeepSeek 最便宜的模型和 Opus 扣得一样多（它*就是*人民币
兜底行）；换算着除，则每个人的配额会随汇率漂。一个 token 计数承载不了价格。想让配额跟着价格
走，就用**按金额**的上限。
...
- **价格表算不出来就是 0**，所以上游不是这两家的一定要自己加行——按 token 的配额照常算（它不看价格表），但账会记不出来。
- 种子里**缺哪行补哪行**，不覆盖你改过的行。补进去的行 `effective_from` 回填到 1970，这样历史记录也能按它重新计价；否则「现在才生效」的价格对过去的账单不可见，重算会一条都改不动（实测：66 条，0 条变化）。
- **历史金额会重算一次**（启动时，只跑一次）。token 数不动，只有金额是派生的，重新派生就是全部动作。日志里会打印改了多少条、前后各是多少。
