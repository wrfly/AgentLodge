...
| **锁定项**（只读） | `retry` 和 `maxConcurrent` 必须是 0，界面和接口都改不了 |
...
**配额与用量**
- 管理员为每个用户配置额度上限、周期（**日/周/月/总量**）、是否硬性拦截
- **重置时刻可配**：月度不必是 1 号、周不必是周一，都能精确到小时
...
**记忆（对标 Claude.ai）**
- 写在 `/memory` 的内容落成 `workspaces/<userId>/MEMORY.md`，并镜像成 `CLAUDE.md` / `AGENTS.md`
- 会话工作目录是它们的子目录，两个 CLI **用各自的原生机制自动加载** —— 不是提示词注入，
  不占对话历史，改完立刻对下一轮生效
