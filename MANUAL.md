...
**配额算的是这一轮实际花了多少钱**，不是数了多少个 token。没有开关，一直是这样。

理由：同一家厂商的价格能差十倍——Fable 5.1 每百万 \$10/\$50，Haiku 4.5 是 \$1/\$5。按 token 数
一视同仁的话，跑贵模型的人花掉十倍的钱，扣掉一样多的配额。

换算方式是：这一轮按价格表算出金额，再除以**兜底行（`*`）一个输入 token 的价格**。所以
「一个计费 token」= 「按标准价的一个输入 token」，每个模型按自己价格的比例折算。兜底行每张表
都有，也不会随着模型增减而变，所以不需要配置任何东西。
...
缓存读那一列是重点：别的模型都是输入价的十分之一，**Fable 是四十分之一**。一个全局的
「缓存读权重」写不出这个差别，一张价格表可以。缓存写是通行的 1.25 倍输入价。
...
- **整张表的币种必须统一**，金额是直接相加的。币种在「系统设置 → 配额 → 币种」，默认 USD；改它只是换标签，不做换算。
- **价格表算不出来的时候退回固定权重**（缓存读 0.1、输出 1.5），不会变成免费。表填错不该让某个模型白跑。
- 种子只在**价格表为空**时写入。已有的部署不受影响，币种也会按表里现有的行钉住，不会因为默认值改成 USD 就把 CNY 的数字重新贴上美元标签。
