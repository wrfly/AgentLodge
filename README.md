...
- [x] 第一个账号永远是管理员，不能降级或停用；其他人注册后在用户列表里设为管理员
- [x] 登录失败按 IP + 邮箱双维度锁定

**计量与配额**
- [x] agent 的上游请求强制经网关，逐次调用从 SSE 流里旁路解析 usage
- [x] 配额闸门在 turn 内部生效；日 / 周 / 月 / 总量周期，重置时刻精确到小时
- [x] 全局并发闸门（默认 ≤3 in-flight），用户级轮转 + AIMD 自适应，后台热调且**落库不丢**；
      AIMD 可关，关掉后闸门恒等于你设的那个数
- [x] 按 token 或按金额计费，价格表可改且历史账单锁定当时价格
- [x] **每个用户看到的是自己的额度**，不是共享订阅的池子——响应头按其配额重写；
      没设上限的用户，用全局用量把池子的利用率折算成他自己的那一份
- [x] **管理员单独看得到上游套餐的真实利用率**与重置时刻
- [x] **每个订阅按它自己真有的周期计费**：5 小时和周这两个滚动窗口是 Claude 订阅的东西，
      只在有上游报过时才强制；Cursor 是月度美元池，按 Cursor 自己报的周期报消费，
      闸门拦人只看月度
...
- [x] **CLI 状态栏显示自己的额度**（`5h 37% · 7d 12%`）——`/usage` 面板问的是
      `api.anthropic.com`、`ANTHROPIC_BASE_URL` 对它无效，所以数字改走响应头进状态栏
