...
export const config = {
   ...
   *
   * Distinct from the ping above, which stays silent while the upstream does. Cloudflare
   * closes a proxied request that produces no response for 120 seconds (524), and that
   * clock starts when the request is forwarded: the queue, the wait for upstream headers,
   * and a stream the ping will not touch all write nothing. Before a real frame exists
   * this writes Codex's comment, because Claude's `event: ping` cannot be the first
   * event; after one, each wire gets the ping it already understands. The first write
   * commits the status — past that, a refusal is said in the stream. 0 turns it off.
   */
